# Kshitij (KJ) Jhunjhunwala

Software engineer at **Composio** in San Francisco (Member of Technical Staff), building agent tooling: the Composio CLI, the Claude Code and Codex agent plugins, and our Stripe Projects integration.

Previously founder of **ProfitLabs**, a cross-exchange analytics platform for prediction markets. Built it solo in under 7 weeks, grew it to around $120M in daily tracked volume and 5,000+ users, wound it down in June 2026.

Before that: Software Engineer at **Cylerity** (healthcare fintech), Consultant Intern at **Deloitte** (global mobility tax).

B.S. Computer Science, Data Science, and Entrepreneurship from **UW-Madison**.

### Selected work

Write-ups, since most of the code is private.

- **[ProfitLabs](https://kshitij-jhunjhunwala.com/build/profitlabs)**: cross-exchange prediction markets terminal. Around $120M in daily tracked volume across Polymarket and Kalshi, 5,000+ users, built solo in under 7 weeks.
- **[Kalshi trading system](https://kshitij-jhunjhunwala.com/build/kalshi)**: two autonomous bots on a shared execution engine. 25x in the first week live, backtested across 12,775 markets.
- **[AI sports research engine](https://kshitij-jhunjhunwala.com/build/ai-sports)**: Claude orchestrating 22 custom SQL tools over 11 seasons of NBA and NFL data.

**Connect:** [LinkedIn](https://www.linkedin.com/in/kshitij-jhunjhunwala11) · kshjhun@gmail.com · [kshitij-jhunjhunwala.com](https://kshitij-jhunjhunwala.com)
